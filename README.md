...
- 登入 / 註冊：`/login`、`/signup`
- 產品工作台：`/app`
- 受保護功能頁：`/app/documents`、`/app/usage`、`/app/company` 等
- 舊網址如 `/usage`、`/documents` 會自動轉址到 `/app/*`
...
- 也會檢查 `/welcome` 是否回到 `/`，以及未登入的 `/app/documents`、`/usage` 是否確實落到 `/login`。
- 如果 live bundle hash 與 `frontend/dist/assets/index-*` 不一致，優先排查部署流程或快取，而不是先懷疑前端路由程式碼。
...
### 多租戶管理
- 每家公司擁有獨立的空間、資料庫記錄與向量索引（Pinecone per-tenant namespace 隔離）
...
- SSO redirect_uri 伺服器端白名單驗證
- 三層速率限制（IP / 使用者 / 租戶）
- 管理角色（superuser / owner / admin / HR）強制 MFA 才能執行特權操作
- 前端 RoleGuard 路由守衛 + 後端統一 DI 權限檢查

### 企業知識庫
- 文件上傳 → 解析 → 切片 → 向量化 → 存入 Pinecone（雲端向量資料庫），全流程背景處理（Celery）
...
- LLM 答案生成：Google Gemini 根據檢索內容生成回答，僅引用提供資料，不自行捏造
...
### 安全防護（Phase 10 — 2026-03-24 完成）
- **HttpOnly cookie session**：存取 token（30 分鐘）、刷新 token（30 天）均為 HttpOnly cookie，取代 localStorage
- **CSRF 中介軟體**：自動驗證 cookie 認證的狀態變更請求（雙重提交 pattern）
- **SSRF 阻擋**：`parse_url()` 解析外部 URL 前驗證 DNS 解析結果，封鎖 Private / Loopback / Link-local / Reserved IP
- **SSO redirect_uri 白名單**：伺服器端驗證所有 OAuth 回調 URI
- **生產環境啟動阻擋器**：`config.py` 在 production/staging 下硬性檢查 SECRET_KEY、DB 密碼、超級管理員帳密、Email 服務、DB SSL、ClamAV、Admin IP 白名單、CORS wildcard — 任何不合格即 `ValueError` 阻擋啟動
- **CORS 限縮**：方法（GET/POST/PUT/PATCH/DELETE/OPTIONS）與標頭（Content-Type/Authorization/X-CSRF-Token/X-Requested-With）明確白名單
- **Payment webhook 安全**：錯誤不外洩資料庫內部資訊
- **安全評估（A- / 8.8 分）**：詳見 [docs/SECURITY_ASSESSMENT_2026-03-23.md](docs/SECURITY_ASSESSMENT_2026-03-23.md)

### 檢索引擎強化（Phase 11 — 2026-03-24 完成）
- **pgvector 雙寫**：文件嵌入同時寫入 Pinecone + PostgreSQL pgvector，確保 fallback 可用
- **零向量排除**：嵌入失敗的 chunk（零向量）不寫入 Pinecone/pgvector，metadata 標記 `embedding_failed`
- **pgvector 語意降級**：Pinecone 斷路器開啟或服務異常時，自動切換至 pgvector HNSW cosine 搜尋
- **BM25 索引快取**：模組級 per-tenant 快取（5 分鐘 TTL + 文件變更失效），消除每次查詢 O(N) 全量載入
- **Native-first 解析策略**：先用免費 native parser → 品質 >= 0.7 直接採用、< 0.7 才升級 LlamaParse；image 格式仍 LlamaParse 優先（handwriting OCR 差距大）
...
└────────────│─────────────────│──────────────────────────────┘
     ...
     └───────────────┘  └─────┬──────┘
                              │
┌─────────────┐  ┌───────────────┐  ┌────────────┐  ┌────────────┐
│ PostgreSQL  │  │    Redis      │  │ Voyage AI  │  │ Gemini     │
│   15        │  │ 7 (queue+     │  │ Embedding  │  │ Flash      │
...
| 類別 | 技術 | 版本 |
|------|------|------|
| Web 框架 | FastAPI | 0.115.9 |
| ORM | SQLAlchemy | 2.0.27 |
| 資料庫遷移 | Alembic | 1.13.1 |
| 資料庫 | PostgreSQL | 15 |
| 快取 / 訊息佇列 | Redis | 7 |
| 背景任務 | Celery | 5.3.6 |
| 向量資料庫 | Pinecone（雲端向量資料庫） | 6.0+ |
| Embedding 模型 | Voyage AI (`voyage-4-lite`, 1024 維) | 0.3.7 |
| LLM | Google Gemini (`gemini-3-flash-preview`) | 1.x (google-genai) |
| 文件解析 | LlamaParse（智慧解析 10 種格式）+ 原生解析器降級 | 0.6.0+ |
| 中文分詞 | jieba（BM25 詞級分詞） | 0.42.1+ |
| 認證 | JWT (python-jose) + HttpOnly cookie session + CSRF + OAuth 2.0 SSO | — |
| 金流 | Stripe Webhook + 藍新金流 NewebPay（AES-256-CBC） | — |
| Email | Resend / SendGrid / SMTP（Celery 異步寄送） | — |
| LLMOps | Langfuse（可觀測性追蹤） | — |
| MCP | MCP Server（外部 AI 整合協議） | 1.0+ |
...
| 生產安全 | HttpOnly cookie + CSRF + 啟動阻擋器（config.py 硬性 ValueError）+ SSRF 防護 + 非 root 執行 |
...
| 模式 | 說明 | 適用場景 |
|------|------|----------|
| `semantic` | Pinecone 向量餘弦相似度 → pgvector fallback（cosine distance） | 語意理解、概念匹配 |
| `keyword` | BM25Okapi 關鍵字匹配（模組級索引快取） | 精確詞彙、編號、法條查詢 |
...
查詢 → HyDE 查詢擴展（語意/混合模式）
       ↓
       ┬─ 語意檢索 (Pinecone + Voyage Embedding → pgvector cosine fallback)
       └─ BM25 關鍵字檢索 (jieba 詞級分詞，模組級索引快取)
              ↓
       ...
       結果（含 content / score / source / metadata）
              ↓
       LLM 答案生成（Google Gemini，僅引用檢索內容）
```
...
- **LLM 答案生成**：Google Gemini 根據檢索結果生成回答，嚴格限制僅引用提供資料
...
- **pgvector Fallback**：Pinecone 斷路器開啟或不可用時，自動降級至 PostgreSQL pgvector HNSW 語意檢索
- **零向量排除**：嵌入失敗的 chunk 不寫入 Pinecone/pgvector，避免汙染語意結果
- **BM25 索引快取**：模組級 per-tenant BM25 索引快取（5 分鐘 TTL），文件變更時自動失效，避免每次查詢全量載入
...
| 向量資料庫 | Pinecone（雲端向量資料庫），以 namespace 進行 per-tenant 隔離 |
...
| 註冊 | `/signup` | 公開註冊頁，與首頁共用導覽/頁尾 |
| SSO 回調 | `/login/callback` | OAuth 2.0 回調處理 |
| Landing | `/` | 公開首頁，含 Hero、功能介紹、FAQ、聯絡我們 |
| 定價方案 | `/pricing` | 公開方案對比與定價 |
| AI 問答 | `/app` | SSE 串流對話、Markdown 渲染、來源面板、回饋按鈕、後續建議、對話搜尋、匯出 Markdown |
| RAG 儀表板 | `/app/rag-dashboard` | 對話量統計、延遲 P50/P95、回饋分布、每日趨勢圖表（管理員） |
| 知識庫管理 | `/app/documents` | 拖放上傳、狀態追蹤、部門篩選 |
| 我的用量 | `/app/my-usage` | 個人使用統計（所有角色） |
| 公司設定 | `/app/company` | 租戶基本資訊、SSO 設定 |
| 部門管理 | `/app/departments` | 樹狀層級結構、可收合節點 |
| 用量統計 | `/app/usage` | Token / 查詢 / 儲存用量圖表 |
| 稽核日誌 | `/app/audit` | 操作紀錄查詢、篩選、匯出 |
| SSO 設定 | `/app/sso-settings` | Google / Microsoft OAuth 設定 |
| 品牌設定 | `/app/branding` | Logo / 色彩 / 名稱 + 即時預覽 |
| 訂閱方案 | `/app/subscription` | 方案對比 + Modal 確認升級 |
| 自訂域名 | `/app/custom-domains` | 域名 CRUD + DNS 驗證引導 |
| 品質儀表板 | `/app/quality-dashboard` | 文件品質分佈 + 檢索得分分佈 + 低品質文件警示（管理員） |
| 接受邀請 | `/accept-invite` | 邀請連結加入組織 |
| Email 驗證 | `/verify-email` | Email 驗證跳轉 |
| 隱私政策 | `/privacy` | PDPA 隱私政策頁 |
| 服務條款 | `/terms` | 服務條款頁 |
| 區域資訊 | `/app/regions` | 資料駐留區域 + 合規資訊 |
...
| 模組 | 路由 | 功能 |
|------|------|------|
| Auth | `/auth/login`, `/auth/register`, `/auth/refresh`, `/auth/logout` | JWT 登入 / 註冊 / token 刷新 / 登出（cookie 清除） |
...
| 端點 | 方法 | 說明 |
|------|------|------|
| `/api/v1/payment/checkout` | POST | 藍新金流（NewebPay）結帳表單 |
| `/api/v1/payment/notify` | POST | 藍新金流 Webhook 通知 |
| `/api/v1/payment/return` | POST | 付款完成返回 |
| `/api/v1/stripe-webhook` | POST | Stripe Webhook（checkout / invoice / subscription） |
| `/api/v1/billing/records` | GET | 帳單記錄查詢 |
...
| `監控指標` | 監控指標收集 | 9090 | 僅內部訪問 |
| `監控頁面` | 監控儀錶板 | 3000 | 強制密碼，禁止註冊 |
...
- ? `app/config.py` 啟動阻擋器：13 項硬性檢查（SECRET_KEY / DB 密碼 / ClamAV / CORS / SSL 等）
- ? HttpOnly cookie session + CSRF 雙重提交防護（取代 localStorage JWT）
- ? 管理角色強制 MFA
- ? SSRF 封鎖（URL 解析前驗證 DNS 解析 IP）
- ? SSO redirect_uri 伺服器端白名單

### Nginx Gateway 多域名路由

```
app.yourdomain.com       → frontend + backend API
admin.yourdomain.com     → admin-frontend + admin-api
監控頁面.yourdomain.com   → 監控頁面 儀錶板
*.yourdomain.com         → 租戶自訂子網域
```
...
```
FastAPI metrics → 監控指標 (scrape) → 監控頁面 (dashboard)
                                     → 告警服務 (alerts → Slack/Email)
```
...
```bash
# 啟動所有容器（web / db / redis / worker / frontend / nginx / 監控指標 / 監控頁面 等 12 個）
...
| ? POSTGRES_SSL_MODE 設為 `require` 或 `verify-full` | ? |
| ? CLAMAV_ENABLED=true | ? |
| ? ADMIN_IP_WHITELIST_ENABLED=true | ? |
| ? EMAIL_PROVIDER 已設定 | ? |
| ? SSO_ALLOWED_REDIRECT_URIS 已配置（如使用 SSO） | ? |
...
| `PINECONE_API_KEY` | Pinecone 向量資料庫 API Key | ? | — |
| `監控頁面_PASSWORD` | 監控頁面 管理員密碼 | ? | `admin` |
...
| `GEMINI_TEMPERATURE` | LLM 生成溫度 | — | `0.3` |
| `LLM_MAX_TOKENS` | LLM 最大輸出 Token | — | `1500` |
...
| `PINECONE_INDEX_NAME` | Pinecone Index 名稱 | — | `aihr-vectors` |
...
| `RETRIEVAL_MIN_SCORE` | 相似度過濾門檻（低於此分數的 chunk 不送入 LLM） | — | `0.35` |
...
| `監控頁面_ROOT_URL` | 監控頁面 外部 URL | — | `https://監控頁面.yourdomain.com` |
...
> ?? **生產環境安全提示**：  
> `app/config.py` 含 `model_validator`，在 `APP_ENV=production` 或 `staging` 時自動進行 **13 項硬性檢查**，任何不合格即 `raise ValueError` 阻擋啟動：
> - `SECRET_KEY` 不可為已知弱預設值或 < 32 字元
> - `POSTGRES_PASSWORD` 不可為 `"postgres"` 或空白
> - `FIRST_SUPERUSER_EMAIL` 不可為 `"admin@example.com"`
> - `FIRST_SUPERUSER_PASSWORD` 不可為 `"admin123"`
> - `EMAIL_PROVIDER` 必須設定
> - `POSTGRES_SSL_MODE` 不可為 `"disable"` 或 `"prefer"`
> - `CLAMAV_ENABLED` 必須為 `true`
> - `ADMIN_IP_WHITELIST_ENABLED` 必須為 `true`
> - `BACKEND_CORS_ORIGINS` 不可包含 `*` 萬用字元
...
| 測試檔案 | 測試項目 | 數量 |
|----------|----------|------|
| `test_auth_security.py` | 認證安全（cookie / CSRF / SSRF / SSO / 生產設定） | 17 |
...
| 服務 | 用途 | 取得網址 | 費用 |
|------|------|----------|------|
| Google Gemini | LLM 生成 + HyDE 查詢擴展 | https://aistudio.google.com/apikey | 免費額度 + 依用量計費 |
| Voyage AI | Embedding (voyage-4-lite) + Rerank (rerank-2) | https://www.voyageai.com/ | 前 100M tokens 免費 |
| Pinecone | 向量資料庫（chunk 儲存 + 語意搜尋） | https://www.pinecone.io/ | 免費 Starter 方案可用 |
...
│   │   │   ├── tenant_admin.py    #   租戶管理員（含 Quality Dashboard + 品牌驗證）
...
│   │   └── billing.py             # ★ 帳單記錄（Stripe / NewebPay）
│   ├── schemas/                   # Pydantic Schemas
│   ├── core/
│   │   ├── redis_client.py        # ★ Redis 執行緒安全單例 + 輕量熔斷器（Phase 9）
│   │   └── cookie_auth.py         # ★ HttpOnly cookie session 管理 + CSRF 雙重提交（Phase 10）
│   ├── crud/                      # 資料存取層
│   ├── services/                  # 核心業務邏輯
│   │   ├── document_parser.py     #   多格式解析引擎（LlamaParse + 原生，~1320 行）
│   │   ├── kb_retrieval.py        #   進階檢索引擎（jieba + HyDE，~595 行）
│   │   ├── chat_orchestrator.py   #   問答協調器（LLM 生成 + Guardrail + 降級）
│   │   ├── core_client.py         #   Core API 客戶端
│   │   ├── newebpay.py            #   藍新金流加解密與交易處理
│   │   ├── email_service.py       #   Email 服務（Resend/SendGrid/SMTP + _esc 防注入）
│   │   ├── langfuse_client.py     # ★ Langfuse LLMOps 追蹤
│   │   ├── subscription.py        #   訂閱方案管理
...
│           ├── QualityDashboardPage.tsx # ★ 品質儀表板（文件/檢索品質）
│           ├── CustomDomainsPage.tsx # ★ 自訂域名 CRUD
│           ├── PricingPage.tsx      # ★ 公開定價方案頁
│           ├── LandingPage.tsx      # ★ 公開首頁
│           ├── SignupPage.tsx       # ★ 自助註冊
│           ├── AcceptInvitePage.tsx  # ★ 邀請連結加入組織
│           ├── VerifyEmailPage.tsx   # ★ Email 驗證
...
├── app/
│   └── mcp_server.py              # ★ MCP Server（外部 AI 整合介面）
...
| Phase 10 | 資安全面強化：HttpOnly cookie session + CSRF 雙重提交 + SSRF 封鎖 + SSO 白名單 + 啟動阻擋器 + 管理角色 MFA（11 項） | ? 完成 |
...
| 項目 | 說明 | 狀態 |
|------|------|------|
| P13-1 | 文件處理進度追蹤：Redis 即時進度（`doc_progress:{id}`）+ REST API（單檔 + 批次） | ? |
| P13-2 | Quality Dashboard 後端：`/company/quality/dashboard` 端點，彙整文件品質與檢索品質 | ? |
| P13-3 | Quality Dashboard 前端：KPI 卡片 + 圓餅圖 + 柱狀圖 + 低品質文件表 | ? |
| P13-4 | 品牌設定驗證：色碼 `#RRGGBB` 正則 + URL 協議白名單（https/http） | ? |
| P13-5 | LLM Guardrail：`_guardrail_block_reason()` Prompt Injection 攔截 + `_sensitive_content_reason()` 敏感資訊篩選 | ? |
| P13-6 | 金流整合：Stripe Webhook（冪等性檢查）+ 藍新金流 NewebPay（AES-256-CBC + SHA256） | ? |
| P13-7 | Email 服務：Resend / SendGrid / SMTP 三後端 + Celery 異步 + HTML 注入防護（`_esc()`） | ? |
| P13-8 | MCP Server：公開 `hr_knowledge_search` / `hr_policy_qa` / `list_documents` 工具 | ? |
| P13-9 | Langfuse LLMOps：解析成本 + Embedding token + RAG 端到端追蹤（trace_id 回傳） | ? |
| P13-10 | Celery 調優：worker concurrency / max-tasks / memory limit / prefetch / acks_late / retry backoff | ? |
| P13-11 | Redis 連線強化：密碼 + socket timeout + keepalive + health check interval | ? |

### Phase 12 任務清單（LLMOps 可觀測性 + MCP Server — 完成）

| 項目 | 說明 | 狀態 |
|------|------|------|
| P12-1 | Langfuse 整合：`langfuse_client.py` 封裝，追蹤文件解析 / Embedding / RAG 查詢全鏈路 | ? |
| P12-2 | MCP Server：`mcp_server.py` 透過 MCP 協議讓外部 AI 調用 UniHR 知識庫 | ? |
| P12-3 | Chat API 回傳 `langfuse_trace_id`，支援逐筆追蹤 | ? |

### Phase 11 任務清單（檢索引擎強化 — 完成）

| 項目 | 說明 | 狀態 |
|------|------|------|
| P11-1 | pgvector 雙寫：文件嵌入同時寫入 Pinecone + PostgreSQL pgvector HNSW | ? |
| P11-2 | pgvector 語意降級：Pinecone 斷路器開啟時自動切換至 pgvector cosine 搜尋 | ? |
| P11-3 | 零向量排除：嵌入失敗 chunk 標記 `embedding_failed`，不寫入索引 | ? |
| P11-4 | BM25 索引快取：per-tenant 模組級快取（5 分鐘 TTL + 文件變更失效） | ? |
| P11-5 | Native-first 解析策略：品質 >= 0.7 採用 native，< 0.7 升級 LlamaParse | ? |

### Phase 10 任務清單（資安全面強化 — 完成）

**觸發**：安全評估報告 [SECURITY_ASSESSMENT_2026-03-23.md](docs/SECURITY_ASSESSMENT_2026-03-23.md) 識別的所有高 / 中風險項目已於 2026-03-24 全數修補，評分從 B+ / 7.5 提升至 **A- / 8.8**。

| 項目 | 說明 | 狀態 |
|------|------|------|
| SEC-22 | HttpOnly cookie session（後端）：建立 `cookie_auth.py` 統一 cookie 管理，更新 `deps.py` / `auth.py` / `sso.py` | ? |
| SEC-23 | HttpOnly cookie session（前端）：`api.ts` / `auth.tsx` 移除所有 localStorage token，改用 `withCredentials` + CSRF header | ? |
| SEC-24 | CSRF 雙重提交中介軟體：建立 `csrf.py` middleware，註冊於 `main.py` | ? |
| SEC-25 | Access Token 生命週期縮短：480 分→ 30 分，前端自動靜默刷新 | ? |
| SEC-26 | 管理角色 MFA 強制：`deps_permissions.py` 新增 `_ensure_privileged_mfa()` | ? |
| SEC-27 | SSRF 防護：`document_parser.py` 新增 `_validate_external_url()` DNS 解析 + IP 驗證 | ? |
| SEC-28 | SSO redirect_uri 白名單：`sso.py` 新增 `_validate_redirect_uri()` | ? |
| SEC-29 | 生產環境啟動阻擋器擴充：`config.py` warnings → ValueError，新增 ClamAV / Admin IP / CORS wildcard 檢查 | ? |
| SEC-30 | CORS 明確白名單：`main.py` 方法與標頭限縮 | ? |
| SEC-31 | Payment webhook 錐誤處理：`payment.py` bare raise → HTTPException(500) | ? |
| SEC-32 | 安全回歸測試：`test_auth_security.py` 17 項測試（cookie / CSRF / SSRF / SSO / 生產設定） | ? |
...
| AI-6 | LLM 答案生成（Google Gemini + Fallback 模板） | P0 | ? |
...
| 文件 | 說明 |
|------|------|
| [docs/SECURITY_ASSESSMENT_2026-03-23.md](docs/SECURITY_ASSESSMENT_2026-03-23.md) | 資安評估報告（A- / 8.8 分，含修補狀態） |
...
| [docs/PRODUCT_INTRODUCTION.md](docs/PRODUCT_INTRODUCTION.md) | 產品介紹 |
| [docs/ENTERPRISE_AI_STRATEGY.md](docs/ENTERPRISE_AI_STRATEGY.md) | 企業 AI 策略 |
| [docs/SAAS_PRODUCTION_READINESS_PLAN.md](docs/SAAS_PRODUCTION_READINESS_PLAN.md) | SaaS 上線準備計畫 |
| [docs/COMMERCIALIZATION_GAP_ANALYSIS.md](docs/COMMERCIALIZATION_GAP_ANALYSIS.md) | 商業化差距分析 |
| [docs/SECURITY_WHITEPAPER.md](docs/SECURITY_WHITEPAPER.md) | 安全白皮書 |
| [docs/BACKUP_RESTORE_DRILL.md](docs/BACKUP_RESTORE_DRILL.md) | 備份還原演練 |
